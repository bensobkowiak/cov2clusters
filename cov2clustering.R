...
cov2clusters <- function(metafile, snpMatrix, beta = c(3.240189, -1.653673, 4.390355),
                         SNPthreshold = 4, includeDate = TRUE, dateThreshold=NULL,  
                         includeSpatial = FALSE, CSVfile=FALSE){
  ...
  snpMatrix<-read.table(snpMatrix)
  
  #order metaData by snpMatrix
  metaData<-metaData[which(metaData[,1] %in% colnames(snpMatrix)),]
  meta.order<-integer()
  for (i in 1:nrow(metaData)){
    meta.order[i]<-which(colnames(snpMatrix)==metaData[i,1])
  }
  metaData<-metaData[order(meta.order),]
  
  # Set up input data list
  dataInput<-list()
  dataInput$IDs<-colnames(snpMatrix)
  dataInput$SNPs<-snpMatrix
  if (includeDate){dataInput$dates<-lubridate::decimal_date(as.Date(metaData[,2]))}
  if (includeSpatial){dataInput$spatial<-metaData[,3]}
  dataInput$trans<-matrix(NA,ncol = ncol(snpMatrix),nrow = nrow(snpMatrix))
  ...
  for (i in seq(length(dataInput$IDs)-1)){
    for (j in seq(i+1, length(dataInput$IDs))){
      covariates<-integer()
      covariates[1]<-1
      covariates[2]<-dataInput$SNPs[i,j]
